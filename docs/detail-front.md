...
상점 구현을 진행하면서 카드의 일부분만 따로 수정하기보다는, 필요한 모든 값을 한 번에 읽어온 후, 작업이 성공적으로 끝났을 때 그 결과를 반영하는 방식이 훨씬 더 안정적이라는 점을 경험했습니다. 특히, 검색 조건이 달라질 때마다 페이지를 1로 초기화하도록 설계한 것도 같은 이유에서 비롯된 선택이었습니다. 이러한 접근법 덕분에 데이터의 일관성을 확실하게 유지할 수 있었고, 예상하지 못했던 오류 발생도 크게 줄일 수 있었습니다.
