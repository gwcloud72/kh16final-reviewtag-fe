...
export default function AdminInventory() {
    ...
    const loadMembers = useCallback(async () => {
        try {
            const resp = await axios.get("/api/admin/inventory/list", { params: { keyword: keyword || null, page } });
    ...
    const loadGrantItems = useCallback(async () => {
        try {
            const resp = await axios.get("/api/admin/inventory/item-list", { params: { page: grantItemPage, size: 10 } });
    ...
    const loadGrantIcons = useCallback(async () => {
        try {
            const resp = await axios.get("/api/admin/icon/list", { params: { page: grantIconPage } });
    ...
    const openManageModal = async (user) => {
        ...
        try {
            const [resInv, resIcon] = await Promise.all([
                axios.get(`/api/admin/inventory/${user.memberId}`),
                axios.get(`/api/admin/icon/${user.memberId}`)
            ]);
    ...
    const handleGrant = async (type, targetNo, name) => {
        try {
            await axios.post(type === "item" ? `/api/admin/inventory/${selectedUser.memberId}/${targetNo}` : `/admin/icon/${selectedUser.memberId}/${targetNo}`);
            Swal.fire({ ...adminSwal, icon: 'success', title: '지급 완료', timer: 1000, showConfirmButton: false });
            const [resInv, resIcon] = await Promise.all([
                axios.get(`/api/admin/inventory/${selectedUser.memberId}`),
                axios.get(`/api/admin/icon/${selectedUser.memberId}`)
            ]);
            setInventoryList(resInv.data || []);
            setIconList(resIcon.data || []);
        } catch (err) { 
