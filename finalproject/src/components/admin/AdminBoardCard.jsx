...
export default function AdminBoardCard({ boardData, refreshList, onActionComplete }) {
    ...
    const toggleEtcDetails = async () => {
        if (!isEtcOpen && etcDetails.length === 0) {
            try {
                ...
                const res = await axios.get(`/api/admin/board/${boardData.boardNo}/reports`);
    ...
    const loadBoardText = useCallback(async () => {
        const { data } = await axios.get(`/api/admin/board/${boardData.boardNo}/text`);
        setBoardText(data);
    }, [boardText]);

    const deleteReport = useCallback(async () => {
        const response = await axios.delete(`/api/board/report/${boardData.boardReportId}`);
        if(response) {
