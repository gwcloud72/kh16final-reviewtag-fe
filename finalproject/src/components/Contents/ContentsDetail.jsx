...
export default function ContentsDetail() {
    ...
    const loadContentsIcons = useCallback(async () => {
        ...
        try {
            const { data } = await axios.get(`/api/icon/contents/${contentsId}`);
    ...
    useEffect(() => {
        ...
        const fetchMyReview = async () => {
            const { data } = await axios.get(`/api/review/user/${contentsId}/${loginId}`);
    ...
    const loadReview = useCallback(async () => {
        setIsLoading(true);
        try {
            const { data } = await axios.get(`/api/review/list/${contentsId}`);
    ...
    const loadBoard = useCallback(async () => {
        const { data } = await axios.get(`/api/board/contentsId/${contentsId}/five`);
    ...
    function ReviewItem({ review, loginId }) {
        ...
        useEffect(() => {
            ...
            const fetchProfile = async () => {
                try {
                    const res = await axios.get(`/api/member/profile/${review.reviewWriter}`);
                    setData(res.data);
                } catch (err) {
