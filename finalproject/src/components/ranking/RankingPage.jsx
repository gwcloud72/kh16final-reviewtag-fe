    ...
    const fetchAllRankings = useCallback(async () => {
        try {
            ...
            const [quizRes, reviewRes, hotRes, liveRes] = await Promise.all([
                axios.get("/api/ranking/quiz?limit=5"),
                axios.get("/api/ranking/review?limit=5"),
                axios.get("/api/ranking/hotContents?limit=5"),
                axios.get("/api/ranking/realtimeQuiz?limit=5")
            ]);
