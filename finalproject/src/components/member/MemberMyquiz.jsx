...
export default function MemberMypage(){
    ...
    const loadData = useCallback(async()=>{
        if (!loginId)  return; 
        const answerList = await axios.get(`/api/member/myanswerquiz/${loginId}/${answerPage}`);
        setAnswerQuizList(answerList.data.list);
        setAnswerPageData(answerList.data.pageVO);
        const addList = await axios.get(`/api/member/myaddquiz/${loginId}/${addPage}`);
        setAddQuizList(addList.data.list);
        setAddPageData(addList.data.pageVO);
        const rateList = await axios.get(`/api/member/myanswerRate/${loginId}`);
