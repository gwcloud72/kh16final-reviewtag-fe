...
export default function BoardDetail() {
    ...
    const loadData = useCallback(async () => {
        try {
            const { data } = await axios.get(`/api/board/detail/${boardNo}`);
            setBoard(data);
    ...
    const loadProfile = useCallback(async () => {
        ...
        try {
            const { data } = await axios.get(`/api/member/profile/${board.boardWriter}`);
            setData(data);
        } catch (err) {
        ...
        const fetchProfileImages = async () => {
            ...
            await Promise.all(
                writersToFetch.map(async (writer) => {
                    try {
                        const res = await axios.get(`/api/member/profile/${writer}`);
